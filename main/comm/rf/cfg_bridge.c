...
/* -------------------------------------------------------------------------
 * Networking bridge — removed with the half firmware.
 *
 * The config tunnel to a "smart" keyboard rode on ESP-NOW, which went away with
 * the halves (their firmware now lives in the Niphargus repo). The chunking and
 * reassembly logic above is pure and host-tested, so it is kept for whatever
 * transport comes next; only the ESP-NOW plumbing is gone.
 *
 * The dongle CDC dispatcher still calls the two predicates below, so they stay
 * as no-ops: with no transport, every command is handled locally.
 * ----------------------------------------------------------------------- */
#ifndef TEST_HOST
#if CONFIG_KASE_DEVICE_ROLE_DONGLE
bool cfg_bridge_have_smart_kbd(void) { return false; }
